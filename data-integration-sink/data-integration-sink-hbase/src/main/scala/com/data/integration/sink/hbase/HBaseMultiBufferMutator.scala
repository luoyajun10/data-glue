package com.data.integration.sink.hbase

import com.data.integration.core.constant.Constants
import com.data.integration.core.model.{Insert, Operation, Update}
import com.data.integration.sink.common.BufferWriter
import com.data.integration.sink.hbase.util.PrimaryKeyUtil
...
class HBaseMultiBufferMutator(config: HBaseConfiguration) extends BufferWriter {
  ...
  def mutate(line: Operation): Unit = {
    ...
    } else if (line.isInstanceOf[com.data.integration.core.model.Delete]) {
      // 删除数据时delete
